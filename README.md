...
公开页面只导出学号、各周已交状态和统计更新时间，不包含姓名，不导出邮件明细、附件路径及 Excel 备注。公开仓库中的网页数据及历史版本可被查看。
...
本项目仅负责学号 **大于 2600015446** 的学生（不含 2600015446）。收取附件、名单导入、Excel 统计及网页导出均按此范围筛选；已有本地历史记录和附件保留。
...
Excel 第一列为姓名、第二列为学号，之后按实际作业周次列出。每次收作业时添加本次指定周次或邮件记录中识别到的周次，保留原有周列，不自动补齐中间周次。例如第1、2周没有作业，则从“第3周”开始。网页只展示 Excel 中已有的周列。名单外学生和姓名不一致会在备注中标注。“邮件明细”保存所有历史周次的处理记录。空白可能表示未交，也可能尚未收取该周数据。
...
双击 `site/index.html` 可本地预览，支持手机布局、学号搜索和周次筛选。更新网页模板后也需要重新运行此命令。页面更新时间来自源 Excel 的修改时间。
